...
#
# G1 23DoF environment class, adapted from TienKungEnv.
...
class G1Env(VecEnv):
    """Environment class for Unitree G1 23DoF humanoid robot with AMP support."""
    ...
    def init_buffers(self):
        ...
        # --- G1-specific body and joint ID lookups ---
        ...
        # Left arm: 5 joints (including wrist_roll)
        ...
        # Right arm: 5 joints (including wrist_roll)
        ...
        # Ankle joints for regularization
        ...
        # End-effector offset vectors (G1 forearm length ~0.25m)
    ...
    def visualize_motion(self, time):
        """Update the robot simulation state based on the AMP motion data at a given time."""
        ...
        # G1 motion frame layout (from gmr_data_conversion output):
        # [0:3] root_pos, [3:6] root_euler,
        # [6:12] left_leg_pos, [12:18] right_leg_pos, [18:23] left_arm_pos, [23:28] right_arm_pos, [28] waist_pos
        # [29:32] root_lin_vel, [32:35] root_ang_vel,
        # [35:41] left_leg_vel, [41:47] right_leg_vel, [47:52] left_arm_vel, [52:57] right_arm_vel, [57] waist_vel
        dof_pos[:, self.left_leg_ids] = visual_motion_frame[6:12]
        dof_pos[:, self.right_leg_ids] = visual_motion_frame[12:18]
        dof_pos[:, self.left_arm_ids] = visual_motion_frame[18:23]
        dof_pos[:, self.right_arm_ids] = visual_motion_frame[23:28]
        dof_pos[:, self.waist_ids] = visual_motion_frame[28:29]

        dof_vel[:, self.left_leg_ids] = visual_motion_frame[35:41]
        dof_vel[:, self.right_leg_ids] = visual_motion_frame[41:47]
        dof_vel[:, self.left_arm_ids] = visual_motion_frame[47:52]
        dof_vel[:, self.right_arm_ids] = visual_motion_frame[52:57]
        dof_vel[:, self.waist_ids] = visual_motion_frame[57:58]
        ...
        root_pos[2] += 0.05  # Small offset to avoid ground penetration
        ...
        # Compute end-effector positions for AMP obs
        left_hand_pos, right_hand_pos, left_foot_pos, right_foot_pos = self._compute_end_effector_pos()
        ...
        return torch.cat(
            (
                self.right_arm_dof_pos,   # 5
                self.left_arm_dof_pos,    # 5
                self.right_leg_dof_pos,   # 6
                self.left_leg_dof_pos,    # 6
                self.waist_dof_pos,       # 1
                self.right_arm_dof_vel,   # 5
                self.left_arm_dof_vel,    # 5
                self.right_leg_dof_vel,   # 6
                self.left_leg_dof_vel,    # 6
                self.waist_dof_vel,       # 1
                left_hand_pos,            # 3
                right_hand_pos,           # 3
                left_foot_pos,            # 3
                right_foot_pos,           # 3
    ...
    def _compute_end_effector_pos(self):
        """Compute end-effector positions relative to root in body frame."""
        left_hand_pos = (
    ...
    def init_obs_buffer(self):
        if self.add_noise:
            ...
            noise_vec[9 + self.num_actions * 3 :] = 0.0  # gait phase
    ...
    def get_amp_obs_for_expert_trans(self):
        """Gets amp obs from current policy state. Returns 58-dim vector for G1."""
        left_hand_pos, right_hand_pos, left_foot_pos, right_foot_pos = self._compute_end_effector_pos()
