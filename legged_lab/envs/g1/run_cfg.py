...
#
# G1 23DoF Run task configuration.
...
from legged_lab.assets.g1 import G1_23DOF_CFG
...
class GaitCfg:
    ...
    gait_phase_offset_l: float = 0.55
    gait_phase_offset_r: float = 0.05
    gait_cycle: float = 0.4


@configclass
class G1RunRewardCfg:
    ...
    feet_slide = RewTerm(
        ...
        weight=-0.25,
        params={
            "sensor_cfg": SceneEntityCfg("contact_sensor", body_names=["left_ankle_roll_link", "right_ankle_roll_link"]),
    ...
    joint_deviation_arms = RewTerm(
        ...
        params={
            "asset_cfg": SceneEntityCfg(
                "robot", joint_names=[".*_shoulder_roll_joint", ".*_shoulder_yaw_joint", ".*_wrist_roll_joint"]
            )
        },
    )
    ...
    feet_y_distance = RewTerm(func=mdp.feet_y_distance, params={"threshold": 0.18}, weight=-2.0)


@configclass
class G1RunFlatEnvCfg:
    amp_motion_files_display = ["legged_lab/envs/g1/datasets/motion_visualization/run.txt"]
    device: str = "cuda:0"
...
@configclass
class G1RunAgentCfg(RslRlOnPolicyRunnerCfg):
    seed = 42
    device = "cuda:0"
    ...
    run_name = ""
    logger = "tensorboard"
    neptune_project = "g1_run"
    wandb_project = "g1_run"
    resume = False
    load_run = ".*"
